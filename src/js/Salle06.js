...
export default class Salle06 extends Phaser.Scene {
  constructor() {
    super({ key: "Salle06" });
  }
  ...
  update() {
    if (open_porte1 == false && Phaser.Input.Keyboard.JustDown(interact) == true &&
      ...
      this.time.delayedCall(500, () => {
        this.scene.start("Couloir1", { x: 3136, y: 1920 });
      });
  ...
  spawnChest() {
    ...
    this.physics.add.overlap(player, this.chestZone, () => {
      if (Phaser.Input.Keyboard.JustDown(interact) && !chest_opened) {
        ...
        this.add.text(this.chest.x, this.chest.y - 50, "+Vitesse de tir !", { fontSize: '16px', fill: '#fff' }).setOrigin(0.5);
        chest_opened = true;

        let fireRate = this.registry.get('fireRate');
        fireRate = Math.max(50, fireRate - 80);
        this.registry.set('fireRate', fireRate);
      }
    });
  }
